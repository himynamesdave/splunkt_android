...
public class CompetitionFragment extends BaseListFragment {
	...
	public void init(View view) {
		...
		this.textViewHeader.setText("Select All Products Mentioned");
	...
	// private void showCategoryFragment(Tournament tournament) {
	// CategoryListFragment.tournament = tournament;
	// MainFragmentActivity
	// .openFragmentbackStack(Constants.CategoryListFragment);
	// }
	...
	public void initListViewAdapter() {
		...
		if (Constants.competitionList == null)
			Constants.competitionList = new ArrayList<String>();

		adapter = new ListViewSplunkCollectionCustomizeAdapter(getActivity(),
				Constants.competitionList);
		...
		listView.setOnItemClickListener(new OnItemClickListener() {
			...
			public void onItemClick(AdapterView<?> parent, View view,
				...
				// TODO Auto-generated method stub
				// showCategoryFragment(Constants.tournamentList.get(position));

				checkCompetition(Constants.competitionList.get(position));
			}
		});
	}
	...
	public void getAllData() {
		...
		if (Constants.competitionList != null
				&& Constants.competitionList.size() > 0) {
			adapter.setData(Constants.competitionList);
			adapter.notifyDataSetChanged();
		} else {
			getAllTournaments();
		}
	...
	private void getAllTournaments() {
		...
		try {
			leadDataDao.getAllCompetition(new JsonHttpResponseHandler() {
				...
				public void onSuccess(int statusCode, Header[] headers,
					...
					try {
						...
						for (int i = 0; i < response.length(); i++) {
							...
							String addCompetition = jsonObject
									.getString("addCompetition");

							Constants.competitionList.add(addCompetition);
	...
	public void onClickSubmitButton() {
		...
		Constants.leadData.setCompe66on(tempUseCase);

		MainFragmentActivity.openFragmentbackStack(Constants.NotesViewFragment);
