...
public class ShirtSizeFragment extends BaseListFragment {

	private final String tag = "ShirtSizeFragment";
	...
	public void init(View view) {
		...
		this.textViewHeader.setText("Select Shirt Size");
	...
	public void initListViewAdapter() {
		...
		if (Constants.shirtSizeList == null)
			Constants.shirtSizeList = new ArrayList<String>();

		adapter = new ListViewSplunkCollectionCustomizeAdapter(getActivity(),
				Constants.shirtSizeList);
		...
		listView.setOnItemClickListener(new OnItemClickListener() {
			...
			public void onItemClick(AdapterView<?> parent, View view,
				...
				// TODO Auto-generated method stub
				checkCompetition(Constants.shirtSizeList.get(position));
			}
		});
	}
	...
	public void getAllData() {
		...
		if (Constants.shirtSizeList != null
				&& Constants.shirtSizeList.size() > 0) {
			adapter.setData(Constants.shirtSizeList);
			adapter.notifyDataSetChanged();
		} else {
			getAllTournaments();
		}
	...
	private void getAllTournaments() {
		...
		try {
			leadDataDao.getAllShirtSize(new JsonHttpResponseHandler() {
				...
				public void onSuccess(int statusCode, Header[] headers,
					...
					try {
						...
						for (int i = 0; i < response.length(); i++) {
							...
							String addShirtSize = jsonObject
									.getString("addShirtSize");

							Constants.shirtSizeList.add(addShirtSize);

						}

						// Constants.tournamentList = tournamentDao
						// .getTournamentFronJSONArray(response);
						//
						// adapter.setData(Constants.tournamentList);
						// adapter.notifyDataSetChanged();
						 adapter.setData(Constants.shirtSizeList);
						 adapter.notifyDataSetChanged();
	...
	public void onClickSubmitButton() {
		...
		Constants.leadData.setShirt_size(selectedList.get(0));

		MainFragmentActivity.openFragmentbackStack(Constants.UseCaseFragment);
